...
void YCrCb_to_ARGB(uint8_t *YCrCb_MCU[3], uint32_t *RGB_MCU, uint32_t nb_MCU_H, uint32_t nb_MCU_V)
{
   ...
   const __m128 v1_402 = _mm_set1_ps(1.402f);
   const __m128 v0_381834 = _mm_set1_ps(0.381834f);
   const __m128 v0_71414 = _mm_set1_ps(0.71414f);
   ...
   for (i = 0; i < 8 * nb_MCU_V; i++) {
      for (j = 0; j < 8 * nb_MCU_H; j += 4) {
          ...
          *     _mm_set...
          */
         Y  = _mm_set_ps((float)MCU_Y[index + 3], (float)MCU_Y[index + 2], (float)MCU_Y[index + 1], (float)MCU_Y[index + 0]);
         Cb = _mm_set_ps((float)MCU_Cb[index + 3], (float)MCU_Cb[index + 2], (float)MCU_Cb[index + 1], (float)MCU_Cb[index + 0]);
         Cr = _mm_set_ps((float)MCU_Cr[index + 3], (float)MCU_Cr[index + 2], (float)MCU_Cr[index + 1], (float)MCU_Cr[index + 0]);
          ...
          */
         Rf = _mm_add_ps(_mm_mul_ps(_mm_sub_ps(Cr, v128), v1_402), Y);
         Bf = _mm_add_ps(_mm_mul_ps(_mm_sub_ps(Cb, v128), v1_7772), Y);
         Gf = _mm_sub_ps(_mm_sub_ps(Y, _mm_mul_ps(_mm_sub_ps(Cb, v128), v0_381834)), _mm_mul_ps(_mm_sub_ps(Cr, v128), v0_71414));
          ...
          */
         __m128i tmp0, tmp1;
         tmp0 = _mm_unpacklo_epi8(Rc, Gc);
         tmp1 = _mm_unpacklo_epi8(Bc, v0);
